...
$all = file_get_contents('https://api.sadjad.ac.ir/v2/stu/grades?' . http_build_query($login));
$json = json_decode($all);

if ( $json->meta->message == 'OK' ) {

    $out = '💯 معدل (بدون درنظر گرفتن نمرات خام): ' . '`' . $json->data->average->average . '`' . "\n\n";
    foreach($json->data->grades as $item) {
        $out .=  '🔻 نام درس: ' . '`' . $item->course_name . '`' . "\n";
        $out .=  '🔻 تعداد واحد: ' . '`' . $item->credits . '`' . "\n";
        $out .=  '🔻  استاد:  ' . '`' . $item->teacher . '`' . "\n";
        $out .=  '🔻  نمره:  ' . '`' . $item->grade . '`' . "\n";
        $out .=  '🔻  وضعیت نمره:  ' . '`' . $item->grade_status . '`' . "\n";
        $out .=  '🔻  وضعیت لیست نمره:  ' . '`' . $item->status . '`' . "\n\n";
    }
